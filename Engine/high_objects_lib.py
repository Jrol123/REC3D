...
class BoundedPlaneParameters(Parameters):
    def __init__(self, pos: Point, rotation: Vector,
                 u, v, du, dv):
...
class CubeParams(Parameters):
    ...
    def scaling(self, value):
        ...
        for i in range(len(self.edges)):
            self.edges[i].scaling(value)
            if i % 2 == 0:
                self.edges[i].pos = self.pos \
                                       + rotations[i // 2].point
            else:
                self.edges[i].pos = self.pos \
                                       - rotations[i // 2].point
    ...
    def rotate(self, x_angle=0, y_angle=0, z_angle=0):
        ...
        # rotations = [self.rot, self.rot2, self.rot3]
        for i in range(len(self.edges)):
            self.edges[i].rotate(x_angle, y_angle, z_angle)
            if i % 2 == 0:
                self.edges[i].pos = self.pos \
                                       + self.edges[i].rot.point
            else:
                self.edges[i].pos = self.pos \
                                       + self.edges[i].rot.point
...
class BoundedPlane(BoundedPlaneParameters):
    def __init__(self, pos: Point, vc_normal: Vector, du, dv):
        """
        ...
        if vc_normal.point == y_dir.point \
                or vc_normal.point == -1 * y_dir.point:
            y_dir = Vector.vs.basis[0]

        u = (vc_normal ** y_dir).normalize()
        v = (vc_normal ** u).normalize()

        super().__init__(pos, vc_normal, u, v, du, dv)
...
class Sphere(SphereParams):
    def __init__(self, pos: Point, rotation: Vector, radius):
        super().__init__(pos, rotation.normalize() * radius, radius)
...
class Cube(Object):
    def __init__(self, pos: Point, rotation: Vector, size: float):
        ...
        self.limit = size / 2
        self.rot = rotation.normalize() * self.limit
        ...
        self.rot2 = (self.rot ** y_dir).normalize() * self.limit
        self.rot3 = (self.rot ** self.rot2).normalize() * self.limit
